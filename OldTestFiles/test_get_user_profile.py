...
class TestGetUserProfile(unittest.TestCase):
    ...
    def setUp(self):
        # url
        self.url = "http://192.168.88.216:8080/orionboot/userProfile"
    ...
    # test the function for input valid token
    ...
    def test_profile_valid_token(self):
        ...
        response = requests.request("GET", self.url, json=payload, headers=headers)
        print(response.text)
        result = response.json()
        # self.assertEqual(result['customer']['active'], 1)

        # for test report
        if self_definition_report:
            expection_result = 1
            test_result = result['customer']['active']
    ...
    # test the function for input used valid token
    # @unittest.skip('to open')
    def test_profile_used_token(self):
        payload = {"order": {
            "lineItemInOrderList": [
                {
                    "product": {
